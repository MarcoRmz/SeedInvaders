...
Sound::Sound(char *fileName){
    ...
    fp=fopen(path,"r");                                            //Open the WAVE file
    ...
    fread(buf,1,dataSize,fp);                                   //Read the sound data and display the
...
Sound::Sound(string filePath, char *fileName){
    string aux = getParentPath(filePath);
    char *cstr = &aux[0u];
    path  = strcat(cstr, fileName);
